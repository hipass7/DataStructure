...
bool Imp_IsBST(TreeNode*, ItemType&, ItemType&);
...
bool Imp_IsBST(TreeNode* tree, ItemType& min, ItemType& max) {
    // 이 tree가 Binary Search Tree인지 아닌지 True/False로 반환하는 함수이다.
    // Post: 재귀함수로 계속 확인을 한다. 왼쪽 노드가 NULL이 아니면 왼쪽 subtree가 BST인지 체크한다.
    //       오른쪽 노드가 NULL이 아니면 오른쪽 subtree가 BST인지 체크한다.
    //       반드시 현재 노드의 info가 왼쪽 subtree에 있는 값보다 큰지도 체크해야한다.
    bool isBST;
    if (tree == NULL)
        return true;

    if (tree->left != NULL) {
        ItemType left_max = tree->info;
        isBST = Imp_IsBST(tree->left, min, left_max);
        if (!isBST || tree->info <= min)
            return false;
    }

    if (tree->right != NULL) {
        ItemType right_min = tree->info;
        isBST = Imp_IsBST(tree->right, right_min, max);
        if (!isBST || tree->info >= max)
            return false;
    }
